...
public class Controller extends BorderPane {
    ...
    private void gameDescribtion() {
        ...
        info.setText("To play the game, use Arrows Keys and press the space button to pause the game" + "\n" + " to control the snake movement game ends if it hits the Wall or collides with itself. "
                + "\n"+ "\n"
                + "Author : Sedanur SEVILMISDAL s19020");

        header.setFill(Color.BLUE); // COLUR OF ABOUT THE GAME / oyunun rengi
        info.setTextAlignment(TextAlignment.LEFT);
        ;
        info.setTextOrigin(VPos.BASELINE);
        info.setFill(Color.BLACK); // Explanation of game colour / yazinin rengi
    ...
    private void setUpOptionBoard() {
        setUp = new VBox();
        setUp.getChildren().addAll(newGame,exit);
    ...
    /********
     ** the action are set up to exit button
     */
